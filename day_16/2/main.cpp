...
luint dfs(luint target, luint etarget, luint time, luint res)
{
    if (time == 0)
        return res;
    ...
    if ( target != etarget && !vvss[target] && vfs[target] > 0 && !vvss[etarget] && vfs[etarget] > 0)
    {
        vvss[target] = true;
        vvss[etarget] = true;
        luint tres = res + (time-1) * (vfs[target] + vfs[etarget]);
        max = std::max(max, dfs(target,etarget,time-1,tres));
        vvss[target] = false;
        vvss[etarget] = false;
    }

    if (!vvss[target] && vfs[target] > 0 && !(!vvss[etarget] && vfs[etarget] > 0))
    {
        vvss[target] = true;
        luint tres = res + (time-1) * vfs[target];
        for (luint & ec : vcs[etarget])
    ...
    {
        vvss[etarget] = true;
        luint tres = res + (time-1) * (vfs[etarget]);
...
luint run(std::string const filename)
{
    ...
    luint res = dfs(start_pos,start_pos,full_time,0);
    return res;
}
