...
class Node
{
    public:
        ...
        Node(std::string name) : name{name}, size{size}, parent{parent} {};
        std::string name;
        luint size = 0;
...
luint run(std::string const filename)
{
    ...
    std::string line; 
    getline(ifs, line);
    std::shared_ptr<Node> root = std::make_shared<Node>("/");//first line is always cd /
    ...
    rec_size(root);
    print_all(root, "");
    return sum_smaller_equal(root, 100000);
}
